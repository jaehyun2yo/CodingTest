  ...
  return s
    ...
    .map((value) =>
      value
        .split("")
        .map((x, i) =>
          i == 0 || i % 2 == 0 ? x.toUpperCase() : x.toLowerCase()
        )
        .join("")
    )
    .join(" ");
...
문자열이니깐 split 를 통해서 배열로변환한다 다만 문자열이 띄워쓰기로 나눠야되니 띄워쓰기 기준으로 나눠주고

## map을 통해 나눠진 배열의 문자열에 접근해서 그 문자열을 다시 인덱스로 접근해야되기때문에 이중 map을 통해서 접근 다시 접근한 배열의 문자열에 split 로 나눠준후 map 을 통해서 i 가 0 이거나 i 인덱스가 짝수라면 해당값에 toUpperCase 로 대문자로 바꾼다 그리고 join으로 다시 문자열로 변환하면끝!
...
s문자열의 길이조건을 맞춘뒤 문자열을 숫자형으로바꿨을때 문자열과 같다면 숫자형이고
만약에 문자가 섞인 문자열을 숫자로 바꾸게된다면 NaN 이 출력되기때문에 이방법으로 풀이를 했다.
